...
const ROUTES = [
  ...
  },
  {
    ...
    title: 'HTTP Status Codes Explained (With Real Log Examples) — OmniLog Blog',
    description: 'Every class of HTTP status code — 2xx to 5xx — explained with the access-log lines you actually see, how to spot scanners in 4xx floods, and how to read the status distribution at a glance.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/grep-awk-log-analysis',
    title: 'Analysing Logs With grep and awk: A Practical Cookbook — OmniLog Blog',
    description: 'The grep and awk patterns that actually come up in log analysis: filter, count, rank top offenders, sum and average fields, work with timestamps, and run a real investigation end to end.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/docker-container-logs',
    title: 'How to View and Manage Docker Container Logs — OmniLog Blog',
    description: 'Master docker logs: follow live output, filter by time, find where logs live on disk, avoid the disk-filling trap with rotation, and export container logs for analysis.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/mysql-slow-query-log',
    title: 'The MySQL Slow Query Log: Finding the Queries That Hurt — OmniLog Blog',
    description: 'Enable and read the MySQL slow query log, decode Rows_examined vs Rows_sent to spot missing indexes, summarise with mysqldumpslow, and diagnose culprits with EXPLAIN.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/dmesg-kernel-log-guide',
    title: 'Reading the Kernel Log: A Practical Guide to dmesg — OmniLog Blog',
    description: 'Read the kernel ring buffer with dmesg: human-readable timestamps, severity filtering, and recognising OOM kills, disk I/O errors, read-only remounts, link flaps, and kernel panics.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/detect-ssh-brute-force-auth-log',
    title: 'How to Detect SSH Brute-Force Attacks in auth.log — OmniLog Blog',
    description: 'Read auth.log like an analyst: spot failed passwords, invalid users, and the one Accepted line that signals a breach. Command-line hunting plus how to shut brute-force attacks down.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/journalctl-cheat-sheet',
    title: 'journalctl Cheat Sheet: Querying the systemd Journal — OmniLog Blog',
    description: 'The journalctl reference you will actually keep open: filter by service, time, boot, and priority, export logs, manage journal size, and combine filters for targeted queries.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/logrotate-configuration-guide',
    title: 'How logrotate Works: Configuring Log Rotation on Linux — OmniLog Blog',
    description: 'A complete guide to logrotate: every directive in a real config explained, the open-file-handle bug that breaks rotation, copytruncate vs postrotate, and how to test safely.',
    subtitle: 'Blog',
  },
  {
    path: '/blog/fail2ban-log-analysis',
    title: 'How fail2ban Reads Your Logs to Block Attackers — OmniLog Blog',
    description: 'Understand fail2ban as a log parser with consequences: jails, filters and failregex, the maxretry/findtime/bantime window, testing filters, and tuning without locking yourself out.',
    subtitle: 'Blog',
  },
  {
